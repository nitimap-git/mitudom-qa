...
export default function AdminDashboard() {
  ...
  const handleCreateActivity = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await supabase.from('activities').insert({ indicator_id: selectedIndicator.id, title: actTitle, description: actDesc, order_index: currentCount + 1 })
      alert('✅ สำเร็จ'); setShowActivityModal(false); setActTitle(''); setActDesc(''); fetchData()
  ...
  // --- Upload Action (แก้ปัญหาปุ่มกดซ้ำ) ---
  const handleUpload = async (e: React.FormEvent) => {
    e.preventDefault()
    if (uploading) return // 🔒 ป้องกันกดซ้ำ
    if (!docTitle) return alert('กรุณาใส่ชื่อเอกสาร')
    
    setUploading(true) // ล็อกปุ่มทันที
  ...
  const handleRemoveFromAlbum = async (docId: number, currentGallery: string[], indexToRemove: number) => {
    ...
    try {
        ...
        if (updatedGallery.length === 0) {
            if (confirm('รูปหมดแล้ว ลบอัลบั้มนี้เลยไหม?')) await supabase.from('documents').delete().eq('id', docId)
            else await supabase.from('documents').update({ gallery: [], file_url: null }).eq('id', docId)
        } else {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8 font-sans">
      <div className="max-w-6xl mx-auto">
                                ...
                                {std.indicators.map((ind: any) => (
                                    <div key={ind.id} className="p-6">
                                        <div className="flex justify-between items-start mb-4">
                                            ...
                                            <button onClick={() => { setSelectedIndicator(ind); setActTitle(''); setActDesc(''); setShowActivityModal(true) }} className="bg-green-600 text-white px-3 py-1.5 rounded text-sm hover:bg-green-700 shadow-sm flex items-center gap-1">+ เพิ่มรายการ</button>
                                        </div>
                                                                    ...
                                                                    <div key={doc.id} className="border-b last:border-0 pb-2 mb-2">
                                                                        <div className="flex justify-between items-center text-sm">
                                                                            ...
                                                                            <div className="flex gap-2 items-center ml-2">
                                                                                {doc.doc_type === 'album' && <button onClick={() => toggleAlbum(doc.id)} className={`text-xs px-3 py-1 rounded transition-colors border ${expandedAlbums[doc.id] ? 'bg-gray-100 text-gray-600' : 'bg-purple-50 text-purple-600 border-purple-100 hover:bg-purple-100'}`}>{expandedAlbums[doc.id] ? 'หุบรูป' : 'ดูรูป 📸'}</button>}
                                                                                <button onClick={() => deleteDoc(doc.id)} className="text-red-500 hover:text-red-700 px-2 py-1 rounded text-xs">✕ ลบ</button>
                                                                            </div>
                                                                        </div>
                                                                        
                                                                        {/* --- ALBUM PREVIEW GRID (ปรับแก้ 10 รูป & ปุ่มลบชัดเจน) --- */}
                                                                        {doc.doc_type === 'album' && expandedAlbums[doc.id] && doc.gallery && (
                                                                            <div className="mt-3 p-3 bg-gray-50 rounded-lg border border-gray-100 animate-fade-in">
                                                                                {/* ปรับ grid-cols-5 เป็น grid-cols-10 สำหรับจอใหญ่ */}
                                                                                <div className="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-10 gap-2">
                                                                                    {doc.gallery.map((url: string, idx: number) => (
                                                                                        <div key={idx} className="relative group/img aspect-square rounded overflow-hidden border border-gray-200">
                                                                                            ...
                                                                                            {/* ปุ่มลบ (เอา opacity-0 ออก ให้เห็นชัดๆ) */}
                                                                                            <button 
                                                                                                onClick={() => handleRemoveFromAlbum(doc.id, doc.gallery, idx)} 
                                                                                                className="absolute top-0 right-0 bg-red-600 text-white w-5 h-5 flex items-center justify-center text-xs shadow hover:bg-red-700"
                                                                                                title="ลบรูปนี้"
                                                                                            >
                                                                                                ✕
                                                                                            </button>
                                                                                    ...
                                                                                    {/* ปุ่มเพิ่มรูป */}
                                                                                    <label className="flex flex-col items-center justify-center aspect-square border-2 border-dashed border-gray-300 rounded cursor-pointer hover:bg-blue-50 hover:border-blue-400 transition text-gray-400 hover:text-blue-600 bg-white">
        ...
        {/* Create/Edit Modals (Code ย่อ) */}
        {showActivityModal && (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50"><div className="bg-white p-6 rounded-lg w-full max-w-md shadow-xl"><h3 className="text-xl font-bold mb-4">เพิ่มหลักฐาน</h3><form onSubmit={handleCreateActivity} className="space-y-4"><div><label className="block text-sm font-bold text-gray-700">ชื่อรายการ</label><input autoFocus value={actTitle} onChange={e => setActTitle(e.target.value)} className="w-full border p-2 rounded text-gray-900" required /></div><div><label className="block text-sm font-bold text-gray-700">คำบรรยาย</label><textarea value={actDesc} onChange={e => setActDesc(e.target.value)} className="w-full border p-2 rounded text-gray-900" rows={3} /></div><div className="flex gap-2 justify-end mt-4"><button type="button" onClick={() => setShowActivityModal(false)} className="px-4 py-2 text-gray-600">ยกเลิก</button><button type="submit" disabled={uploading} className="px-4 py-2 bg-blue-600 text-white rounded font-bold">{uploading ? '...' : 'บันทึก'}</button></div></form></div></div>
        )}
        {showEditActivityModal && (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50"><div className="bg-white p-6 rounded-lg w-full max-w-md shadow-xl border-t-4 border-yellow-500"><h3 className="text-xl font-bold mb-4">แก้ไข</h3><form onSubmit={handleUpdateActivity} className="space-y-4"><div><label className="block text-sm font-bold text-gray-700">ชื่อรายการ</label><input autoFocus value={actTitle} onChange={e => setActTitle(e.target.value)} className="w-full border p-2 rounded text-gray-900" required /></div><div><label className="block text-sm font-bold text-gray-700">คำบรรยาย</label><textarea value={actDesc} onChange={e => setActDesc(e.target.value)} className="w-full border p-2 rounded text-gray-900" rows={3} /></div><div className="flex gap-2 justify-end mt-4"><button type="button" onClick={() => setShowEditActivityModal(false)} className="px-4 py-2 text-gray-600">ยกเลิก</button><button type="submit" disabled={uploading} className="px-4 py-2 bg-yellow-500 hover:bg-yellow-600 text-white rounded font-bold">บันทึก</button></div></form></div></div>
        )}

        {/* --- MODAL UPLOAD (แก้ปุ่มกดซ้ำ) --- */}
        {showUploadModal && (
             <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
                <div className="bg-white p-6 rounded-lg w-full max-w-md shadow-xl">
                    <h3 className="text-xl font-bold mb-2">แนบไฟล์</h3>
                    <p className="text-sm text-blue-600 mb-4 font-semibold">หัวข้อ: {selectedActivity?.title}</p>
                    <form onSubmit={handleUpload} className="space-y-4">
                        ...
                        <div className="flex gap-2 justify-end mt-4">
                             ...
                             {/* ปุ่มล็อกเมื่ออัปโหลด */}
                             <button 
                                type="submit" 
                                disabled={uploading} 
                                className={`px-4 py-2 text-white rounded font-bold ${uploading ? 'bg-gray-400 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700'}`}
                             >
                                {uploading ? '⏳ กำลังอัปโหลด... (กรุณารอ)' : 'ยืนยัน'}
                             </button>
                        </div>
                    </form>
                </div>
